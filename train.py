...
def setup(rank: int, world_size: int, master_port: int) -> None:
    ...
    dist.init_process_group("nccl", rank=rank, world_size=world_size, timeout=timedelta(minutes=180))
...
def train_fn(
    ...
    ratings_file='../month_merge_query/part-*'
...
#             return False
        ...
        for eval_iter, row in enumerate(eval_data_loader):
            seq_features, target_ids, target_ratings = movielens_seq_features_from_row(
...
#     file_handler = logging.FileHandler(f'./log/my_log_file_{rank}.log', mode='w')
