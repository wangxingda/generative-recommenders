...
def train_fn(
    ...
    ratings_file='../month/part-*'
    ratings_file_lists = sorted(glob.glob(ratings_file))
    dataset = get_reco_dataset(
        dataset_name=dataset_name,
        max_sequence_length=max_sequence_length,
        chronological=True,
        positional_sampling_ratio=positional_sampling_ratio,
        ratings_files = ratings_file_lists,
        rank=rank,
...
#     train_data_sampler, train_data_loader = None, None
...
#             if rank ==1:
#                 print(batch_id)
#                 print(row['historical_ids'].shape)
#             print(row)
#             print(row['historical_ids'].shape)
#             print(row['historical_ratings'].shape)
#             print(row['historical_timestamps'].shape)
#             pdb.set_trace()
...
#             )
            ...
            click_mask = seq_features.past_payloads['ratings'][:, 1:] != 0
            ar_mask = torch.logical_and(ar_mask, click_mask)

#             indices = torch.nonzero(click_mask)
#             click_mask_matrix = torch.zeros_like(click_mask, dtype=torch.int32, device=device)
...
#                     row_index = index[0]
#                     column_index = 0
#                 click_mask_matrix[row_index, column_index] = index[1]
#             print('loss_index',click_mask_matrix)
#             pdb.set_trace()
            ...
            if (batch_id % eval_interval) == 0:
                logging.info(
                        ...
                        f" total time: {time.time() - total_training_time:.2f}s "
                    )
                last_training_time = time.time()
                if rank == 0:
...
#                 break
        ...
        logging.info(
            ...
            f"NDCG@10 {ndcg_10:.4f}, NDCG@50 {ndcg_50:.4f}, NDCG@100 {ndcg_100:.4f}, HR@10 {hr_10:.4f}, HR@50 {hr_50:.4f}, HR@100 {hr_100:.4f}"
        )
        last_training_time = time.time()
...
def setup_logging(rank):
    """设置日志记录器配置."""
    logging.basicConfig(
        level=logging.DEBUG,  # 设置全局日志记录级别
        format='%(asctime)s - %(levelname)s - %(message)s'
    )

    # 创建文件处理器，每个进程有独立的日志文件
    file_handler = logging.FileHandler(f'./log/my_log_file_{rank}_SASRec_.log', mode='w')
    ...
    console_handler.setLevel(logging.ERROR)  # 只显示INFO级别以上的日志
    console_handler.addFilter(InfoFilter())  # 添加自定义过滤器，只显示INFO级别的日志
